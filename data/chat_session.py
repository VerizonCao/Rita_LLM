...
class ChatSessionManager:
    ...
    def update_last_image_message_url(
        self, 
        user_id: str, 
        avatar_id: str, 
        old_image_url: str, 
        ...
        try:
            ...
            if not result or len(result) == 0:
                logger.warning(f"No chat session found for user {user_id} and avatar {avatar_id}")
                return None
            
            # Parse messages from JSONB
            messages_data = result[0].get('messages', [])
            from .message import parse_messages_from_jsonb
            messages = parse_messages_from_jsonb(messages_data)
            
            # Find and update the last assistant message with the matching image URL
            updated = False
            ...
            if not updated:
                logger.warning(f"No assistant message found with image URL: {old_image_url}")
                return None
            
            # Convert updated messages back to JSONB
            from .message import messages_to_jsonb
            messages_jsonb = messages_to_jsonb(messages)
            ...
            result = self.db_manager.execute_query(
                update_query, 
                (messages_jsonb, user_id, avatar_id)
            )
            ...
            logger.error(f"Error updating last image message URL: {e}")
            return None 