...
class ChatMessage(ABC):
    ...
    def __init__(
        ...
        Args:
            ...
            origin_image_url: A temporary url generated by s3
        """
...
def update_live_image_url(target_message: ChatMessage) -> bool:
    """
    Update the live image URL of the message.
    
    Args:
        target_message: The message to check
        
    Returns:
        True if the message contains a live image URL, False otherwise
    """
    # Check if message has an image URL
    if not target_message.imageUrl:
        logger.debug(f"Message {target_message.id} has no imageUrl")
        return False
    
    # Check if the imageUrl is an S3 key (not a full URL)
    if target_message.imageUrl.startswith('https://') or target_message.imageUrl.startswith('http://'):
        logger.debug(f"Message {target_message.id} imageUrl is already a full URL")
        return False
    
    # Generate a new presigned URL for the S3 key
    try:
        new_public_url = s3_manager.get_public_url(target_message.imageUrl, expires_in=3600)
        
        if new_public_url:
            # Update the origin_image_url field with the new public URL
            target_message.origin_image_url = new_public_url
            logger.info(f"Updated live image URL for message {target_message.id}: {target_message.imageUrl} -> {new_public_url}")
            return True
        else:
            logger.warning(f"Failed to generate public URL for message {target_message.id} with S3 key: {target_message.imageUrl}")
            return False
            
    except Exception as e:
        logger.error(f"Error updating live image URL for message {target_message.id}: {e}")
        return False